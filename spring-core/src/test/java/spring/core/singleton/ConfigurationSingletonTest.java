...
public class ConfigurationSingletonTest {
    ...
    void configurationDeepTest() {
        ...
        // 부모: milkcoke.core.config.AppConfig, 자식: milkcoke.core.config.AppConfig$$SpringCGLIB
        var bean = ac.getBean(AppConfig.class);

        // bean = class milkcoke.core.config.AppConfig$$SpringCGLIB$$0
        // 클래스 이름이 복잡해진 이유는
        // @Configuration 어노테이션에 의해 CGLIB 이라는 바이트코드가 조작 라이브러리를 사용하여
        // milkcoke.core.config.AppConfig 를 상속받아 별도의 클래스를 스프링 컨테이너 안에 등록한 것이다.
        // 이름은 appConfig 지만 milkcoke.core.config.AppConfig$$SpringCGLIB 이 온 이유는 그것 때문이다.
